...
$(window).scroll(function() {
	if($(document).scrollTop() > $('.parameter_box').offset().top) {
		$('.navfixed').css('display', 'block').css('height', '40');
	} else {
		$('.navfixed').css('height', '0').css('display', 'none');
	};
	for(var a = 0; a < $('.detail_lists').length; a++) {
		if(window.scrollY >= $('.detail_lists')[a].offsetTop) {
			$('.nav_box').eq(0).find('li').eq(a).addClass("on").siblings().removeClass("on");
			$('.nav_box').eq(1).find('li').eq(a).addClass("on").siblings().removeClass("on");
		}
	}
})
$('.nav_box').eq(1).find('li').on('click', function() {
	$(this).addClass("on").siblings().removeClass("on");
	var i = $(this).index()
	$('.nav_box').eq(0).find('li').eq(i).addClass("on").siblings().removeClass("on");
});
$('.nav_box').eq(0).find('li').on('click', function() {
	$(this).addClass("on").siblings().removeClass("on");
	var i = $(this).index()
	$('.nav_box').eq(1).find('li').eq(i).addClass("on").siblings().removeClass("on");
});

//商品参数选项
$('.color_ul li').on('click', function() {
	$(this).addClass("on").siblings().removeClass("on");
})

//数量加
$('.btn-add').click(function() {
	var buynum = $('#buy_num').val() - 0;
	buynum++;
	$('#buy_num').val(buynum)
});
//数量减
...
$.ajax({
	...
	success: function(str) {
		...
		$('.sort1').html(obj.sort1);
		$('.sort2').html(obj.sort2);
		var arr = obj.smallimg.split(',');
		$('.MagTargetImg').attr('src', arr[0])
		var $res = arr.map(function(item) {
			return `<li><img src="${item}" data-lsrc="${item}" data-maxSrc="${item}" /></li>`
		}).join('');
		$('.smallimglist').html($res);
		$('.smallimglist li').eq(0).attr('class', 'on');
	}
});
...
function getmessage() {
	$.ajax({
		...
		success: function(str) {
			...
			var $res = arr.map(function(item) {
				return `<li class="appraise_li">
							<div class="a_li_t1">
								<div class="appr_cons">${item.content}</div>
									<div class="appr_time">${item.dataline}</div>
								</div>
								<div class="a_li_t2">
									*****
								</div>
...
$('#send').click(function() {
	var myDate = new Date();
//	过滤敏感词
//	var message = filter($('#creatMessage').val())
//	console.log(message)
	if(getcookie) {
		if($('#creatMessage').val().trim()) {
			$.ajax({
				type: "post",
				url: "../api/details.php",
				async: true,
